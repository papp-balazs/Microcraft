...
DxgiInfoManager::DxgiInfoManager()
{
	...
	const auto DxgiGetDebigInterface = reinterpret_cast<DXGIGetDebugInterface>(
		reinterpret_cast<void*>(GetProcAddress(hModDxgiDebug, "DXGIGetDebugInterface"))
	);
	FAILED_LAST_EXCEPT_NOINFO("DxgiInfoManager setup error", DxgiGetDebigInterface);

	FAILED_EXCEPT_NOINFO("DxgiInfoManager setup error", DxgiGetDebigInterface(__uuidof(IDXGIInfoQueue), &pDxgiInfoQueue));
}
